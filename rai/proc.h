...
/* Run-time support code for *.g.h C code generated by ai-array-c.rkt

   The *.g.h files contain C struct and function definitions for the
   singal processing function, and a collection of macros capturing
   meta (type) information.

   The data structures and functions define here are intended to
   capture the static information present in the macros, essentially
   providing two programmer interfaces:

   - high-level: using the data structures presented in this file to
     dynamically load and instantiate processor objects.

   - low-level: using macros in the *.g.h to generate statically bound
     C code.

   If there is no to pick the low-level interface, you should probably
   be using the high-level one.
